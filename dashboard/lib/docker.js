...
const MOCK_CONTAINERS = [
  { id: 'mock-npm-001', name: 'ms-npm', image: 'jc21/nginx-proxy-manager:latest', state: 'running', status: 'Up 2 hours', ports: [{ private: 80, public: 80, type: 'tcp' }, { private: 81, public: 81, type: 'tcp' }], created: Date.now() / 1000 },
  { id: 'mock-port-002', name: 'ms-portainer', image: 'portainer/portainer-ce:latest', state: 'running', status: 'Up 2 hours', ports: [{ private: 9000, public: 9000, type: 'tcp' }], created: Date.now() / 1000 },
  { id: 'mock-home-003', name: 'ms-homepage', image: 'ghcr.io/gethomepage/homepage:latest', state: 'running', status: 'Up 2 hours', ports: [{ private: 3000, public: 3000, type: 'tcp' }], created: Date.now() / 1000 },
  { id: 'mock-dash-004', name: 'ms-dashboard', image: 'megastack/dashboard:latest', state: 'running', status: 'Up 2 hours', ports: [{ private: 8443, public: 8443, type: 'tcp' }], created: Date.now() / 1000 },
  { id: 'mock-glue-008', name: 'ms-gluetun', image: 'qmcgaw/gluetun', state: 'running', status: 'Up 45 min', ports: [{ private: 8080, public: 8080, type: 'tcp' }], created: Date.now() / 1000 },
  { id: 'mock-qbit-009', name: 'ms-qbittorrent', image: 'lscr.io/linuxserver/qbittorrent:latest', state: 'running', status: 'Up 44 min', ports: [], created: Date.now() / 1000 },
  { id: 'mock-prow-010', name: 'ms-prowlarr', image: 'lscr.io/linuxserver/prowlarr:latest', state: 'running', status: 'Up 44 min', ports: [], created: Date.now() / 1000 },
  { id: 'mock-sonr-011', name: 'ms-sonarr', image: 'lscr.io/linuxserver/sonarr:latest', state: 'running', status: 'Up 44 min', ports: [], created: Date.now() / 1000 },
  { id: 'mock-radr-012', name: 'ms-radarr', image: 'lscr.io/linuxserver/radarr:latest', state: 'running', status: 'Up 44 min', ports: [], created: Date.now() / 1000 },
  { id: 'mock-jell-013', name: 'ms-jellyfin', image: 'lscr.io/linuxserver/jellyfin:latest', state: 'running', status: 'Up 44 min', ports: [{ private: 8096, public: 8096, type: 'tcp' }], created: Date.now() / 1000 },
];

// List all MegaStack containers (prefixed with ms-)
async function listContainers() {
  if (!docker) return MOCK_CONTAINERS;
  const containers = await docker.listContainers({ all: true });
  return containers
    .filter(c => c.Names.some(n => n.startsWith('/ms-')))
    .map(c => ({
...
// Validate that a container belongs to MegaStack (ms- prefix)
async function validateMegaStackContainer(containerId) {
  if (!docker) {
  ...
  return containers.some(c =>
    ...
    c.Names.some(n => n.startsWith('/ms-'))
  );
}
...
async function restartContainer(containerId) {
  if (!docker) return;
  if (!(await validateMegaStackContainer(containerId))) throw new Error('Container not found');
  const container = docker.getContainer(containerId);
...
async function streamLogs(containerId) {
  if (!docker) {
    ...
    const lines = [
      '2026-02-06T17:00:00Z [INFO] MegaStack demo mode - no Docker socket\n',
