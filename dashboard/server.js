...
const PORT = 8443;
const MS_ROOT = process.env.MS_ROOT || '/opt/megastack';
...
const SESSION_SECRET = process.env.MS_SESSION_SECRET || crypto.randomBytes(32).toString('hex');
...
app.post('/api/login', loginLimiter, async (req, res) => {
  ...
  const storedHash = process.env.MS_ADMIN_PASSWORD_HASH || '';
  ...
  if (!storedHash) {
    ...
    try {
      ...
      await config.update(MS_ROOT, { MS_ADMIN_PASSWORD_HASH: hash });
      process.env.MS_ADMIN_PASSWORD_HASH = hash;
  ...
  try {
    ...
    if (storedHash.startsWith('$2')) {
      ...
      if (valid) {
        ...
        await config.update(MS_ROOT, { MS_ADMIN_PASSWORD_HASH: newHash });
        process.env.MS_ADMIN_PASSWORD_HASH = newHash;
      }
    }
...
app.get('/api/auth/status', (req, res) => {
  const hasPassword = !!process.env.MS_ADMIN_PASSWORD_HASH;
  res.json({
...
app.get('/api/modules', requireAuth, async (req, res) => {
  try {
    const mods = await modules.list(MS_ROOT);
    res.json(mods);
  } catch (err) {
...
// Rich metadata from x-megastack compose extensions (for App Store UI)
app.get('/api/modules/store', requireAuth, async (req, res) => {
  try {
    const mods = await modules.listRich(MS_ROOT);
    res.json(mods);
  } catch (err) {
...
app.post('/api/modules/:name/enable', requireAuth, async (req, res) => {
  try {
    await modules.enable(MS_ROOT, req.params.name);
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/modules/:name/disable', requireAuth, async (req, res) => {
  try {
    await modules.disable(MS_ROOT, req.params.name);
    res.json({ success: true });
  } catch (err) {
...
const ALLOWED_CONFIG_KEYS = new Set([
  'MS_DOMAIN', 'TZ', 'MS_ADMIN_PASSWORD_HASH',
  'VPN_PROVIDER', 'VPN_TYPE', 'VPN_USER', 'VPN_PASSWORD',
  'SERVER_COUNTRIES', 'MEDIA_ROOT',
  'NOTIFIARR_API_KEY',
]);

app.get('/api/config', requireAuth, async (req, res) => {
  try {
    const cfg = await config.read(MS_ROOT);
...
app.put('/api/config', requireAuth, async (req, res) => {
  try {
    ...
    await config.update(MS_ROOT, updates);
    res.json({ success: true });
  } catch (err) {
...
app.post('/api/update', requireAuth, updateLimiter, async (req, res) => {
  try {
    const result = await docker.pullAndRecreate(MS_ROOT);
...
app.post('/api/backup', requireAuth, backupLimiter, async (req, res) => {
  try {
    const file = await backup.create(MS_ROOT);
...
app.get('/api/backups', requireAuth, async (req, res) => {
  try {
    const backups = await backup.list(MS_ROOT);
    res.json(backups);
  } catch (err) {
...
app.get('/api/backups/:filename', requireAuth, async (req, res) => {
  try {
    ...
    if (filename.endsWith('.enc')) {
      ...
      filePath = await backup.decrypt(MS_ROOT, filename);
      const downloadName = filename.replace('.enc', '');
      res.download(filePath, downloadName);
    } else {
      filePath = await backup.getPath(MS_ROOT, filename);
...
server.listen(PORT, '0.0.0.0', () => {
  log('INFO', `MegaStack Dashboard running on port ${PORT}`);
  log('INFO', `MS_ROOT: ${MS_ROOT}`);
  log('INFO', `Backup encryption: ${process.env.MS_BACKUP_KEY ? 'dedicated key' : process.env.MS_SESSION_SECRET ? 'session secret' : 'disabled'}`);
});
